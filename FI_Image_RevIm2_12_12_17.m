...
clear;
IterationNum=1250; %2000;
c_total=2;
PoolIterations=6; %always add plus 1 for random drawing
PoolNum=250; %Number of samples in initial labeled pool
lambdaspan=10^-8; %10^-1; %3.7365e+12; %10.^linspace(-6,1,8);
lambdaIspan=1;
KernelSize=9;

im=rgb2gray(imread('86016.jpg')); %converts truecolor to intensity

seg = readSeg('86016.seg');

[rs,cs]=size(seg);
for i=1:rs %eliminate single pixel segmentations from original image
    for ii=1:cs
        if seg(i,ii)==2
            seg(i,ii)=1; %background is 1
        end
        if seg(i,ii)==3
            seg(i,ii)=2; %circle is 2
        end
        if seg(i,ii)==4
            seg(i,ii)=2;
        end
...
save('circle_delp5_12_13_17.mat','del','-v7.3');
%toc

load('circle_delp5_12_13_17.mat');
%toc
disp('Del Loaded');
    ...
    for lambdaI=lambdaIspan
        ...
        for PoolIteration=1:PoolIterations
            ...
            if mod(PoolIteration,2) == 1 %odd
                ...
                random=0;
            elseif mod(PoolIteration,2) == 0 %odd
                clear PoolIndex;
                clear NewLabels;
            ...
            save('OutputIm2L8_12_12_17.mat','Output','-v7.3');
