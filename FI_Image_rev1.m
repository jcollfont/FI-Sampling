...
image=reshape(im1,(size(im,1)*size(im,2)),1); %make image list (of pixel values)

%% Initial Labeled Pool
[PoolIndex]=datasample(1:(length(image)),PoolNum); %randomly samples w/o replacement
PoolIndex=PoolIndex';
...
NewLabels=zeros(length(image),1); %empty estimated labels
