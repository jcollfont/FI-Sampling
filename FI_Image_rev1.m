...
% toc
% 
% save('bird_del_11_7_17.mat','del','-v7.3');
% toc
...
% div1=11;
%div2=10;
% RecordedAccuracies3=zeros(div1,5);
% q=1;
...
for lambda=10^-4; %10.^linspace(-9,1,div1)
    for lambdaI=10^-4; %10.^linspace(-8,1,div2)
...
%             RecordedAccuracies3(q,1)=lambda;
%             %RecordedAccuracies(q,2)=lambdaI;
%             RecordedAccuracies3(q,3)=accuracyTotal;
%             RecordedAccuracies3(q,4)=accuracy1;
%             RecordedAccuracies3(q,5)=accuracy2;
%             q=q+1;

            figure()
            plot(AccuracyVsIterationTotal)
            title('Total Accuracy')
            xlabel('Iteration #')
            ylabel('Accuracy')
            figure()
            plot(AccuracyVsIterationClass1)
            title('Class 1 Accuracy')
            xlabel('Iteration #')
            ylabel('Accuracy')
            figure()
            plot(AccuracyVsIterationClass2)
            title('Class 2 Accuracy')
            xlabel('Iteration #')
            ylabel('Accuracy')
        end
    end
