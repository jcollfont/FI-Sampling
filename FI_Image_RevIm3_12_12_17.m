...
clear;
IterationNum=1000; %2000;
c_total=2;
PoolIterations=6; %always add plus 1 for random drawing
PoolNum=500; %Number of samples in initial labeled pool
lambdaspan=10^-7; %10^-1; %3.7365e+12; %10.^linspace(-6,1,8);
lambdaIspan=1;
KernelSize=9;

im=rgb2gray(imread('3096.jpg')); %converts truecolor to intensity

seg = readSeg('3096.seg');
...
if size(im,1)>size(im,2)
    ...
    im=im(floor(ldiff):end-1,:);
    seg=seg(floor(ldiff):end-1,:);
    seg2class=seg2class(floor(ldiff):end-1,:);
...
save('plane_delNOISEp6_12_12_17.mat','del','-v7.3');
%toc

load('plane_delNOISEp6_12_12_17.mat');
%toc
disp('Del Loaded');
...
for lambda=lambdaspan
    ...
    for lambdaI=lambdaIspan
        ...
        for PoolIteration=1:PoolIterations
            ...
            save('OutputIm3_12_12_17.mat','Output','-v7.3');
